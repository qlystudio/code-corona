...
-- MAKE TARGET 
local target = display.newCircle( _W/2, _H/2, 50 )
target:setFillColor( 255, 0, 0, 0.5)
physics.addBody( target, "static", {density=1, friction=0.5, bounce=0.2 } )
local targetHolder = display.newGroup()
targetHolder:insert( target )

-- targetHolder:scale(.25, .25)
...
function throw( points, sling, start, distance, angle )
	...
	ball.t = timer.performWithDelay(4000, ball, 1)
