...
-----------------------------------------------------------------------------------------
    ...
    if ( "session" == event.type ) then
		...
		-- THIS IS THE INFORMATION THAT IS DISPLAYED ON A POST ON A FAEBOOK WALL
...
--			facebook.request( "me/friends" )		-- Alternate request
		...
		if fbCommand == POST_PHOTO then
			local attachment = {
				name = "Developing a Smaion! Facebook test share",
				link = "http://www.click3x.com",
		...
		if fbCommand == POST_MSG then
			...
			local postMsg = {
				message = "Posting from Corona SDK and Billy Hayes is pretty cool! " ..
...
-- function getFb() 
 ...
 --    height = 37,
 --    left =-65,
 --    top = (tonumber(startBtn.height)*1.5 + 37),
 --    fontSize = 32,
 ...
 --  })
	...
	-- 	-- "Post Photo with Facebook" button
		...
		{
			defaultFile = "assets/images/ui/global/ico-fb-btn.png",
