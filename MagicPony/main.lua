-- Project: HorseSprite (plus! audio)
-- Version: 1.0 + (gwl)
--
-- Original Author: Japan Corona Group
-- Code alterations:  Garrison L
--
-- Original Sample code is MIT licensed, see http://www.coronalabs.com/links/code/license
...
--[[ Nota Bene:
The fog isn't really working in this code, but
I've not taken the time to fix it yet.  See if
you can toy with it.  It doesn't appear on-screen
until it is already half-way on-screen... probably
has something to do with the graphics 2.0 changes.
garrison
]]

-- A Bright and Shiny silver dollar to anyone who
-- knows the radio play (!) from which the following
-- music is taken.  (That's right:  Radio play.  I was
-- born in 1902, and my family used to gather around the
-- old Victrola every Sunday to listen to the Phil Silvers Show.)
----------------------------------------------------------
...
local journeyMusic = audio.loadStream("sorcerer.mp3")
...
local spriteOptions = { name="uma", start=1, count=8, time=900 }
...
-- start playing the sprite animation (i.e., the 8 horse images)
spriteInstance:play()

-- now start playing the theme song -- so simple!
local singForMe = audio.play(journeyMusic)

-- an addEventListener is executed at "runtime" (e.g.,
	-- the moment you run the code.)  Here, it is set
	-- to execute a custom function called "move" (look for
	-- that code earlier in the program).  It executes move()
	-- during a condition called "enterFrame."  Corona SDK
	-- creates an event called "enterFrame" every time a new
	-- frame is drawn on screen (i.e., usually about 60 times / second).
	-- You will use "enterFrame" a lot.
