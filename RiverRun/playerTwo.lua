...
local background
local background2
local sfx = audio.loadStream("Audio/SoundFX/power-up.mp3")
...
local function onSingleBtnRelease()	
	...
	local sfx = audio.play( sfx, { channel=3, loops=0, fadein=0 }  )  -- play the background music on channel 1, loop infinitely, and fadein over 5 seconds 
	audio.stop(2)
	leave()
...
function scene:enterScene( event )
	...
	local backgroundMusicChannel = audio.play( backgroundMusic, { channel=2, loops=-1, fadein=0 }  )  -- play the background music on channel 1, loop infinitely, and fadein over 5 seconds 
